...
const sendEmail = (formData, files) => {
  ...
  const mailOptions = {
    ...
    \nProducto: ${ProductInformation ? ProductInformation.product : "N/A"}
    \nDescripcion: ${ProductInformation ? ProductInformation.description : "N/A"}
    \nEstado del Producto: ${ProductInformation ? ProductInformation.stateOfProduct : "N/A"}
  ...
  Vendor.create({
    ...
    .then((newVendor) => {
      return sendEmail(
        ...
        .catch((err) => {
          console.error("Error sending email after creating vendor:", err);
